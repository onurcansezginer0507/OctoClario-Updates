...
cvd_all_in_one__v2 <- function(input_dir){
  ...
  data_list_graph <- list(fv_data, fii_data,  c677_data, a1298_data, fxiii_data, pai_data, fgb_data, hpai_data,  apob_data,fvcamb_data, ace_data, lta_data, apoe1_data, apoe2_data, h1299r_data)
  names(data_list_graph) <- c("FV-LEI","FII", "C677T", "A1298C", "FXIII", "PAI", "FGB", "HPAI","APOB",  "FV-CAMB", "ACE", "LTA", "APOE1", "APOE2", "H1299R")
  ...
  if(nrow(list2DF(result_tb_list_12)) != 0){
    ...
    for (i in 1:length(result_tb_list_12)) {
      ...
      for (j in 1:nrow(result_tb_list_12[[i]])) {
        ...
        if ("Pos Ctrl" %in% well_info$Content[which(well_info$Well %in% rownames(result_tb_list_12[[i]])[j])]){
